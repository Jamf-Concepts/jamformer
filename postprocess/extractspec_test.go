...
func TestBuildExtractFileName(t *testing.T) {
	tests := []struct {
		...
		{"raw ppd no existing ext", "HP DeskJet 2600", FileKindRaw, ".ppd", "", "HP DeskJet 2600.ppd"},
		{"raw ppd already has ext", "HP.ppd", FileKindRaw, ".ppd", "", "HP.ppd"},
		{"raw mobileprovision", "Enterprise App", FileKindRaw, ".mobileprovision", "", "Enterprise App.mobileprovision"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := buildExtractFileName(tt.input, tt.kind, tt.rawExt, tt.content, map[string]int{})
			if got != tt.want {
