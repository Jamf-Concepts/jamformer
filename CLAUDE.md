...
4. **platform** — `WriteSingletonImports()` writes `singletons_import.tf` (import id `singleton`) for the ~27 settings resources, then `terraform plan -generate-config-out` materialises their config and merges it into `generated.tf`. Discovery and the singleton plan both drive the spinner progress bar: discovery counts `list_complete` events (one per list type) against the selected-type count via `terraform.QueryProgressFunc`; the singleton plan reuses `terraform.ProgressFunc` (the per-import "Refreshing state..." counter)
...
- Jamf Platform downloads package files from the Jamf Cloud Distribution Service (JCDS) when `JAMF_TENANT_ID` is set and `-skip-package-downloads` is not passed (only JCDS-resident files are fetchable; the rest stay as metadata + server hashes). It still skips icon handling and category_id/-1 removal, but does extract scripts/profiles/app-configs (see the Platform pipeline above)
